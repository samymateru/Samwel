...
@router.get("/{control_effectiveness_rating_id}", response_model=ControlEffectivenessRating)
def fetch_control_weakness_rating(
    ...
    if user.status_code != 200:
        raise HTTPException(status_code=user.status_code, detail=user.description)
    try:
        data = get_control_effectiveness_rating(db, control_effectiveness_rating_id=control_effectiveness_rating_id)
        return data
    except HTTPException as e:
        raise HTTPException(status_code=e.status_code, detail=e.detail)
